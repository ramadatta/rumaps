...
def umapviz(
    ...
    # Convert labels to a Categorical if it's not already
    ...
    # Handle colors and colormap
    label_color_map = None

    # If a custom colormap is provided, it should take precedence
    if custom_cmap:
        cmap = custom_cmap
        print("Using custom colormap provided by the user.")
    # Otherwise, check if colors are available in adata.uns
    ...
    # Fall back to the default colormap
    else:
        cmap = plt.cm.tab20
